...
def train_model(dataloaders,dataset_sizes, model, criterion, optimizer, scheduler, device, num_epochs=25):
    ...
    best_aucs = np.zeros((numClasses,)) #AUCs of each class vs all
    best_auc = 0
    best_epoch = -1
    ...
    print(f"Best model in epoch {best_epoch:d} val AUC OMERACT 0: {best_aucs[0]:.4f} OMERACT 1: {best_aucs[1]:.4f} OMERACT 2: {best_aucs[2]:.4f} OMERACT 3: {best_aucs[3]:.4f} avg: {epoch_auc:.4f}")
